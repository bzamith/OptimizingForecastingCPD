...
from src.forecaster import CNNForecaster, OBSERVATION_WINDOW, hpo_fit
...
def execute_mean_cut(cuts: List[int], cut_seconds: List[float], X_train: pd.DataFrame, X_test: pd.DataFrame, variables: List[str], hpo: bool = False) -> Tuple[pd.DataFrame, pd.DataFrame]:
...
def execute_median_cut(cuts: List[int], cut_seconds: List[float], X_train: pd.DataFrame, X_test: pd.DataFrame, variables: List[str], hpo: bool = False) -> Tuple[pd.DataFrame, pd.DataFrame]:
    total_cut_seconds = sum(cut_seconds)
    cuts.sort()
